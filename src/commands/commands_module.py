...
def bot_commands_admin(bot : commands.Bot,
                       database : PickleDB,
    ...
    def check_admin_command(ctx : commands.Context):
        ...
        if (ctx.guild.id == server_admin_id 
    ...
    async def delete_all(ctx : commands.Context):
        ...
        if status_code :
            ...
            await ctx.send("An error has occurred while purging the database : check logs")
    ...
    async def delete_server(ctx : commands.Context, server_id : str):
        ...
        status_code = await database_module.purge_database_server(database=database, server_id=server_id)
        
        if status_code:
            # Send message confirmation
            await ctx.send(f"Keys of the server id {server_id} has been successfully deleted")
        else:
            await ctx.send(f"There has been some issues with some keys deletion for server id {server_id} : check logs")
    ...
    async def delete_key(ctx : commands.Context, key_id : str):
        ...
        status_code = await database_module.purge_database_key(database=database, key=key_id)
        
        if status_code:
            # Send message confirmation
            await ctx.send(f"Key {key_id} has been successfully deleted")
        else:
            await ctx.send(f"Key {key_id} couldn't be found or deleted from the database : check logs ")
