...
def bot_commands(bot: commands.Bot, database: PickleDB):
    ...
    def embed_quest_list_from_member(
        member: discord.member, list_quests: list
    ) -> discord.Embed:
        ...
        for index, quest in enumerate(list_quests):
            ...
            embed.add_field(
                name=f"{quest_category_emoji} Quête {quest_category_name} {quest_tag_name} #{quest_index}",
    ...
    async def send_message_with_role(
        ctx: commands.Context, member: discord.member, required_role: str, msg: str
    ):
    ...
    async def ajout_quete(
        ctx: commands.Context, quest_label=None, *, quest_comments=""
    ):
        ...
        if quest_label in help_labels:
            ...
            if label_dict is not None:
                ...
                await ctx.reply(
                    f"""Le ou la mercenaire {ctx.author.mention} a ajouté la quête suivante sur le comptoir de quêtes de la guilde :\nCatégorie : Quête {category_translated}\nNom : {label_dict["name"]}\nCommentaires : {quest_comments}"""
                )
                ...
                await ctx.send(
                    f"""Aucune quête n'a été trouvée avec le libellé : {quest_label}"""
                )

    # Command to get all quests from the current user
    @bot.command(
        aliases=["lire", "read"],
        help="Affiches l'ensemble de tes quêtes sous forme d'indices",
    )
    async def lire_quetes(ctx: commands.Context):
        ...
        if list_quests:
            embed = embed_quest_list_from_member(
                member=ctx.author, list_quests=list_quests
            )

            await ctx.reply(embed=embed)
        else:
            await ctx.reply(
                "Il n'y a aucune quête à ton nom sur le comptoir : utilise `!ajout_quete` pour en ajouter une !"
            )

    # Command to get all quests from the current server
    @bot.command(
        aliases=["lire_tout", "read_all"],
        help="Affiches l'ensemble des quêtes de tous les membres du serveur",
    )
    async def lire_toutes_quetes(ctx: commands.Context, quest_label=None):
        ...
        dict_users_quests = await database_module.get_quests_from_server(
            database=database, server_id=server_id, filter_label=None
        )
        ...
        if dict_users_quests:
            ...
            await ctx.reply(
                f"La liste des membres ayant renseigné une quête sur le comptoir est la suivante : {list_user_name}"
            )
            ...
            if quest_label:
                ...
                await ctx.reply(
                    "Il n'y a aucune quête de ce type sur le comptoir : utilise `!ajout_quete` pour en ajouter une!"
                )
            else:
                # Message if there is no quest at all
                await ctx.reply(
                    "Il n'y a aucune quête sur le comptoir : utilise `!ajout_quete` pour être le premier à en ajouter!"
                )

    # Command to remove a quest from the user list (passing an index)
    @bot.command(
        aliases=["del", "supp"],
        help="Supprimes la quête indexé en argument de ta liste de quête",
    )
    async def supp_quete(ctx: commands.Context, arg: int):
        ...
        if isinstance(arg, int):
            ...
            if status_code == 0:
                await ctx.reply(
                    f"La quête de {ctx.author.display_name} numérotée {arg} a été correctement supprimée"
                )
            else:
                await ctx.send(
                    f"La quête de {ctx.author.display_name} numérotée {arg} n'est pas trouvable"
                )
        else:
