...
def bot_commands(bot : commands.Bot, database : PickleDB):
    ...
    def embed_quest_list_from_member(member : discord.member, list_quests : list, hide_index=False) -> discord.Embed:
    ...
    async def send_message_with_role(interaction : discord.Interaction, member : discord.member, required_role : str, msg : str):
    ...
    async def mention_common_members(interaction : discord.Interaction, quest_label = None):
        ...
        if quest_label:
            ...
            list_members_id = [user_id for user_id in dict_users_quests.keys() if user_id != str(interaction.user.id)]
            
            for member_id in list_members_id:
                member = bot.get_guild(int(server_id)).get_member(int(member_id))
                await send_message_with_role(interaction=interaction,
                                             member=member,
    ...
    @app_commands.describe(
        quest_label="Libellé de quête",
        quest_comments="Commentaires optionnels"
    )
    async def ajout_quete(interaction : discord.Interaction, quest_label : str = None, quest_comments: str = ""):
        ...
        if (quest_label in help_labels):
            ...
            if label_dict is not None:
                ...
                quest_embed = embed_quest_list_from_member(interaction.user,[quest_dict])
                ...
                await mention_common_members(interaction=interaction, quest_label=quest_label)
            
            else:
                await interaction.response.send_message(content=f"""Aucune quête n'a été trouvée avec le libellé : {quest_label}""")
            
    # Command to get all quests from the current user
    @bot.tree.command(name="lire_quetes", description="Affiches l'ensemble de tes quêtes ordonnée par indices")
    ...
    async def lire_quetes_logic(interaction: discord.Interaction, followup = False):
        ...
        if list_quests: 
            ...
            embed = embed_quest_list_from_member(member=interaction.user, list_quests=list_quests)
            ...
            if followup:
                await interaction.followup.send(content="Il n'y a aucune quête à ton nom sur le comptoir : utilise `/ajout_quete` pour en ajouter une!")
            else:    
                await interaction.response.send_message(content="Il n'y a aucune quête à ton nom sur le comptoir : utilise `/ajout_quete` pour en ajouter une!")

        
    
    # Command to get all quests from the current server
    @bot.tree.command(name="lire_toutes_quetes", description="Affiches l'ensemble des quêtes de tous les membres du serveur")
    @app_commands.describe(
        quest_label="Libellé de quête"
    )
    async def lire_toutes_quetes(interaction : discord.Interaction, quest_label : str = None): 
        ...
        if dict_users_quests :
            ...
            await interaction.response.send_message(content=f"La liste des membres ayant renseigné une quête sur le comptoir est la suivante : {list_user_name}")
            ...
            if quest_label:
                ...
                await interaction.response.send_message(content="Il n'y a aucune quête de ce type sur le comptoir : utilise `/ajout_quete` pour en ajouter une!")
            else:
                # Message if there is no quest at all
                await interaction.response.send_message(content="Il n'y a aucune quête sur le comptoir : utilise `/ajout_quete` pour être le premier à en ajouter!")  
    
    
    
    # Command to remove a quest from the user list
    @bot.tree.command(name="supp_quete", description="Supprimes la quête indexé en argument de ta liste de quête")
    @app_commands.describe(
        idx="Index de la liste de quête"
    )
    async def supp_quete(interaction : discord.Interaction, idx: int):
        ...
        if isinstance(idx, int):
            ...
            if(quest_label_removed):
                await interaction.response.send_message(f"La quête de {interaction.user.display_name} - {label_module.read_label(quest_label_removed)["name"]} numérotée #{idx} a été correctement supprimée")
                await lire_quetes_logic(interaction=interaction, followup=True)
            else:
                await interaction.response.send_message(f"La quête de {interaction.user.display_name} numérotée {idx} n'est pas trouvable")
        else:
            await interaction.response.send_message("Numéro d'index inconnu on non reconnu : veuillez réessayer")
    
    # Comment to get help
    @bot.tree.command(name="aide", description="Fonction pour afficher le guide d'utilisation")
    async def aide(interaction : discord.Interaction):
        ...
        await interaction.response.send_message(content=label_module.read_help(help_type="/"))
        
    # Autocomplete logic
    
    @ajout_quete.autocomplete("quest_label") 
    @lire_toutes_quetes.autocomplete("quest_label") 
    async def quest_label_autocomplete(
        ...
        list_labels = [{"key": key, "name": value.get('name')} for key, value in dict_label.items()]
        
        return [app_commands.Choice(name=f"{choice.get("key")} - {choice.get("name")}", value=choice.get("key"))
        for choice in list_labels
        if current.lower() in f"{choice.get("key")} - {choice.get("name")}".lower()][:25]
    ...
    async def quest_delete_idx_autocomplete(
        ...
        list_labels_quests = [{"index": index + 1, "name": label_module.read_label(quest["quest_label"])["name"] , "comments": quest["quest_comments"] } for index, quest in enumerate(list_quests)]
        
        return [app_commands.Choice(name=f"#{choice.get("index")} - {choice.get("name")} {choice.get("comments")}", value=choice.get("index"))
